...
const WomenProducts = () => {
  const { products, setProducts, like, cart, setCart, LogUser, handleLike, handleCart } = useContext(mycontext);
  ...
  useEffect(() => {
    const prod = products.filter((data) => data.category === "women");
    setFiltered(prod);
  }, [products]);
  ...
  function handleChange(e) {
    ...
    const filtered = products.filter((pdt) => {
      const { category, name, price, description } = pdt || {};
      return (
        category === "women" &&
        ...
        filtered.map((item) => (
          <div className="zz" key={item._id}>
            ...
            <button onClick={() => likel(item._id)}>
              {like.includes(item._id) ? (
                <img src={heart} alt="Liked" />
              ) : (
                <img src={heart1} alt="Unliked" />
              )}
            </button>
